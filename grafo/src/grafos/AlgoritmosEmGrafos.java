...
public interface AlgoritmosEmGrafos {
    ...
    public static Grafo carregarGrafo(String path, TipoDeRepresentacao t) throws Exception {
        FileManager fileManager = new FileManager();
        ArrayList<String> linhas = fileManager.stringReader(path);

        if (linhas == null || linhas.isEmpty()) {
            throw new Exception("Arquivo vazio");
        }

        int numVertices = Integer.parseInt(linhas.get(0).trim());
        int numArestas = contarArestas(linhas);

        Grafo g = null;

        if (t == TipoDeRepresentacao.MATRIZ_DE_ADJACENCIA) {
            g = new MATRIZ_DE_ADJACENCIA(numVertices);

        } else if (t == TipoDeRepresentacao.MATRIZ_DE_INCIDENCIA) {
            g = new MATRIZ_DE_INCIDENCIA(numVertices, numArestas);

        } else if (t == TipoDeRepresentacao.LISTA_DE_ADJACENCIA) {
            g = new LISTA_DE_ADJACENCIA(numVertices);
        }
            ...
            for (int i = 1; i < linhas.size(); i++) {           
                ...
                if (partes.length < 2) {
                    throw new Exception("Formato de linha inválido: " + linha);
                }

                Vertice origem = new Vertice(Integer.parseInt(partes[0]));
                for (int j = 1; j < partes.length; j++) { // O(n^2)
                    String[] destinos = partes[j].split(";");
                    for (String destinoPeso : destinos) {
                        String[] dp = destinoPeso.split("-");

                        int destinoId = Integer.parseInt(dp[0]);
                        double peso = Double.parseDouble(dp[1]);
    ...
    private static int contarArestas(ArrayList<String> linhas){
        int count = 0;
        for (String linha : linhas) {
            count += linha.split("\\d+-\\d+").length - 1;
        }
        return count;
    }
    ...
    public static void main(String[] args) {
        try {
            String filePath = "src/data/Teste.txt";
            Grafo grafo = carregarGrafo(filePath, TipoDeRepresentacao.LISTA_DE_ADJACENCIA);

            System.out.println(grafo.toString());

            // Faça outras operações com o grafo conforme necessário
        } catch (Exception e) {
